...
import { validateUpload, containsLikelyPromptInjection } from './file-safety.js';
...
export async function ingestDocumentLocal(sql,user,payload){
  ...
  // P0-D: file-safety BEFORE parsing/storage.
  const gate = validateUpload({ filename: rawName, mimeType: payload.mime_type || undefined, sizeBytes: bytes.length });
  if (!gate.ok) { const e = new Error(gate.message); e.code = gate.code; throw e; }
  if (!bytes.length) { const e = new Error('Document is empty'); e.code = 'EMPTY_FILE'; throw e; }
  const filename=gate.safeName,extension=path.extname(filename).slice(1).toLowerCase()
  ...
  if(!bytes.length||bytes.length>12*1024*1024)throw new Error('Document is empty or too large')
  ...
  // Ingested content is untrusted document text — never system instructions. Flag injection-bearing docs.
  const injectionHit = parsed.passages?.some((p) => containsLikelyPromptInjection(p.content));
  if (injectionHit) parsed.injection_flag = true;
