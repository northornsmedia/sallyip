...
| File-safe ingestion | `file-safety.js` wired into `document-ingestion-local.js` + `ingest-document.py` | `file-safety` + `ingestion-routes` 13 tests | node path live-parse test (malformed surfaces) | BLOCKED (python route live-upload not staged) | BLOCKED | 11 attack cases covered at unit; route inventory: 2/2 ingestion paths wired, 34-handler audit separate | INTEGRATION_VERIFIED |
| Origin config | `APP_ORIGIN` prefer + host allowlist, referer env, manifest generator | `activation-wave` origin tests | BLOCKED (prod deploy not reverified) | BLOCKED | BLOCKED | `chat-orchestrator siteUrlFor`, `scripts/generate-word-manifest.mjs`; manifest.xml still committed generated file | IMPLEMENTED |
...
| Audit coverage | matters + embeddings/rerank rejections added (6/34 audited) | audit unit tests | BLOCKED (handler hit-count not staged) | BLOCKED | BLOCKED | inventory: auth, contracts, patent-drafts, playbooks, matters, embeddings, rerank = AUDITED; 27 NOT_AUDITED | IMPLEMENTED |
| Secret scanning | `scripts/secret-scan.mjs` + `secret:scan` | activation test | `node scripts/secret-scan.mjs` clean (this wave) | BLOCKED (no pre-commit hook installed / CI workflow absent) | BLOCKED | history `git log --all -- .env.local` empty; scanner excludes examples | IMPLEMENTED |
...
| Build | Vite | — | `npm run build` clean 7.3s | n/a | local only | dist output present | INTEGRATION_VERIFIED |
