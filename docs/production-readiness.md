...
| Verification | 60 | 90 | P0 release BLOCKED (entailment 66.7%, unsupported 81%, missing quotes 4.2%); P1 services unit-tested, not yet wired into chat finalizer | frozen reports unchanged | `benchmarks/regression_report_p0_full.md`, `contradiction/temporal/entailment` + tests |
...
| Document Security | 55 | 95 | Policy lib exists but not enforced in every upload route (`ingest-document.py`, `generate-file` need wiring); 25MB/200KB split documented | unit tests pass | `src/lib/file-safety.js`, file tests |
