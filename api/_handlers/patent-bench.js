...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'GET') return res.status(200).json(await listPatentBench(sql, { limit: req.query?.limit }))
    if (req.method !== 'POST') return res.status(405).json({ error: { message: 'Method not allowed' } })
    try { requireEditor(user) } catch (error) { return res.status(403).json({ error: { message: error.message } }) }
    return res.status(201).json(await runPatentBench(sql, user.id, (req.body || {}).name))
  } catch (error) {
    return res.status(400).json({ error: { message: error.message || 'PatentBench operation failed' } })
  }
}
