  ...
  try{
    ...
    if(!id){
      if(req.method==='GET'){
        ...
        await logSecurityEvent(sql,{userId:user.id,event_type:'generated_files_list',req,action:'list',resource:'generated_file',result:'ok',severity:'info'}).catch(()=>{});
    ...
    if(req.method==='DELETE'){
      ...
      await logSecurityEvent(sql,{userId:user.id,event_type:'generated_file_delete',req,action:'delete',resource:'generated_file',resource_id:id,result:'ok',severity:'info'}).catch(()=>{});
    ...
    if(!file){
      await logSecurityEvent(sql,{userId:user.id,event_type:'generated_file_access_denied',req,action:'read',resource:'generated_file',resource_id:id,result:'denied',severity:'warn'}).catch(()=>{});
      return res.status(404).json({error:{message:'File not found'}});
    }
    await logSecurityEvent(sql,{userId:user.id,event_type:'generated_file_download',req,action:'download',resource:'generated_file',resource_id:id,result:'ok',severity:'info'}).catch(()=>{});
