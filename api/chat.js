...
export default async function handler(req,res){
  ...
  try{
    ...
    const answer=enforceSourceDisclosure(result.answer,verification)
    return res.status(200).json({id:`sally-${Date.now()}`,object:'chat.completion',model:'sallyip/4.1-pro',choices:[{index:0,message:{role:'assistant',content:answer},finish_reason:'stop'}],sally_meta:{...result.meta,agent_run_id:run.id,route,verification,sources:evidence.map(({content,...source})=>source)}})
